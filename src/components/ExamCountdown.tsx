...
const ExamCountdown = () => {
  ...
  const examDate = new Date("2025-10-15T09:00:00");
...
const ExamCountdown = () => {
  // Use numeric constructor: month is 0-indexed (9 = October)
  const examDate = new Date(2025, 9, 15, 9, 0, 0); // Oct 15, 2025 09:00 local time

  const calculateTimeLeft = () => {
    const now = new Date();
    const difference = examDate.getTime() - now.getTime();
    console.log("now:", now.toISOString(), "exam:", examDate.toISOString(), "diff(ms):", difference);
  ...
  // Show fallback for debugging / when date passed
  if (!timeLeft) {
    return (
      <section className="py-8 text-center">
        <Calendar className="inline-block w-6 h-6 mr-2" />
        <p className="text-base">Exam has started or date is in the past.</p>
      </section>
    );
  }
